...
public class PowerGenerationTestSteps {
    private App app; 
    ...
    public void i_have_a_power_generation_test_setup() throws Exception {
        app = (App) ApplicationContextProvider.getApplicationContext().getBean("app");
       
    }

    @When("^I execute the power generation test with subject \"([^\"]*)\"$")
    public void i_execute_the_power_generation_test_with_subject(String natsSubject) throws Exception {
         response = app.PowerGenerationtest(natsSubject);
